...
(defn new-coord [xy v]
  (let [x (first xy)
        y (second xy)
        direction (first v)
        distance (Integer/parseInt (apply str (rest v)))]
    (cond (= \L direction) [(- x distance) y]
          (= \R direction) [(+ x distance) y]
          (= \U direction) [x (+ y distance)]
          (= \D direction) [x (- y distance)])))
...
(def wire "R75,D30,R83,U83,L12,D49,R71,U7,L72")
(str/split wire #",")

(map (partial new-coord [0 0]) (str/split wire #","))
